{ This file is generated 26.07.2015 13:56:40 by Ygres compiler ver. 14.01.11156 Standard Win32  (build date Jul 20 2015). }
unit _sg_api;
interface
{ Languages }
...
{ ------------------------------------------------------------ }
 ...
 const NOUN_ru: integer = 6;                       { class ясыеярбхрекэмне }
 const PRONOUN2_ru: integer = 7;                   { class леярнхл_ясы }
 const PRONOUN_ru: integer = 8;                    { class леярнхлемхе }
 const ADJ_ru: integer = 9;                        { class опхкюцюрекэмне }
 const NUMBER_CLASS_ru: integer = 10;              { class вхякхрекэмне }
 const INFINITIVE_ru: integer = 11;                { class хмтхмхрхб }
 const VERB_ru: integer = 12;                      { class цкюцнк }
 const GERUND_2_ru: integer = 13;                  { class дееопхвюярхе }
 const PREPOS_ru: integer = 14;                    { class опедкнц }
 const IMPERSONAL_VERB_ru: integer = 15;           { class аегкхв_цкюцнк }
 const PARTICLE_ru: integer = 18;                  { class вюярхжю }
 const CONJ_ru: integer = 19;                      { class янчг }
 const ADVERB_ru: integer = 20;                    { class мюпевхе }
 const PUNCTUATION_class: integer = 21;            { class осмйрсюрнп }
 const POSTPOS_ru: integer = 26;                   { class онякекнц }
 const POSESS_PARTICLE: integer = 27;              { class опхръф_вюярхжю }
 const MEASURE_UNIT: integer = 28;                 { class едхмхжю_хглепемхъ }
...
const NEUTRAL_xx: integer = 2;                     // GENDER : NEUTRAL
 const SPEECH_STYLE_xx: integer = 9;               { enum яРХКЭпЕВХ }
{ Coordiname яРХКЭпЕВХ states: }
 const STRENGTH_xx: integer = 10;                  { enum пЮГЛЕПяХКЮ }
{ Coordiname пЮГЛЕПяХКЮ states: }
 const PERSON_ru: integer = 11;                    { enum кхжн }
{ Coordiname кхжн states: }
const PERSON_1_ru: integer = 0;                    // кхжн : 1
const PERSON_2_ru: integer = 1;                    // кхжн : 2
const PERSON_3_ru: integer = 2;                    // кхжн : 3
 const NUMBER_ru: integer = 12;                    { enum вхякн }
{ Coordiname вхякн states: }
const SINGULAR_NUMBER_ru: integer = 0;             // вхякн : ед
const PLURAL_NUMBER_ru: integer = 1;               // вхякн : лм
 const GENDER_ru: integer = 13;                    { enum пнд }
{ Coordiname пнд states: }
const MASCULINE_GENDER_ru: integer = 0;            // пнд : лсф
const FEMININE_GENDER_ru: integer = 1;             // пнд : фем
const NEUTRAL_GENDER_ru: integer = 2;              // пнд : яп
 const TRANSITIVENESS_ru: integer = 14;            { enum оепеундмнярэ }
{ Coordiname оепеундмнярэ states: }
const NONTRANSITIVE_VERB_ru: integer = 0;          // оепеундмнярэ : меоепеундмши
const TRANSITIVE_VERB_ru: integer = 1;             // оепеундмнярэ : оепеундмши
 const PARTICIPLE_ru: integer = 15;                { enum опхвюярхе }
 const PASSIVE_PARTICIPLE_ru: integer = 16;        { enum ярпюд }
 const ASPECT_ru: integer = 17;                    { enum бхд }
{ Coordiname бхд states: }
const PERFECT_ru: integer = 0;                     // бхд : янбепь
const IMPERFECT_ru: integer = 1;                   // бхд : меянбепь
 const VERB_FORM_ru: integer = 19;                 { enum мюйкнмемхе }
{ Coordiname мюйкнмемхе states: }
const VB_INF_ru: integer = 0;                      // мюйкнмемхе : хгзъб
const VB_ORDER_ru: integer = 1;                    // мюйкнмемхе : онасд
 const TENSE_ru: integer = 20;                     { enum бпелъ }
{ Coordiname бпелъ states: }
const PAST_ru: integer = 0;                        // бпелъ : опньедьее
const PRESENT_ru: integer = 1;                     // бпелъ : мюярнъыее
const FUTURE_ru: integer = 2;                      // бпелъ : асдсыее
 const SHORTNESS_ru: integer = 21;                 { enum йпюрйхи }
 const CASE_ru: integer = 23;                      { enum оюдеф }
{ Coordiname оюдеф states: }
const NOMINATIVE_CASE_ru: integer = 0;             // оюдеф : хл
const VOCATIVE_CASE_ru: integer = 1;               // гбюр
const GENITIVE_CASE_ru: integer = 2;               // оюдеф : пнд
const PARTITIVE_CASE_ru: integer = 3;              // оюпр
const INSTRUMENTAL_CASE_ru: integer = 5;           // оюдеф : рбнп
const ACCUSATIVE_CASE_ru: integer = 6;             // оюдеф : бхм
const DATIVE_CASE_ru: integer = 7;                 // оюдеф : дюр
const PREPOSITIVE_CASE_ru: integer = 8;            // оюдеф : опедк
const LOCATIVE_CASE_ru: integer = 10;              // леяр
 const FORM_ru: integer = 24;                      { enum ндсь }
{ Coordiname ндсь states: }
const ANIMATIVE_FORM_ru: integer = 0;              // ндсь : ндсь
const INANIMATIVE_FORM_ru: integer = 1;            // ндсь : мендсь
 const COUNTABILITY_ru: integer = 25;              { enum оепевхякхлнярэ }
{ Coordiname оепевхякхлнярэ states: }
const COUNTABLE_ru: integer = 0;                   // оепевхякхлнярэ : дю
const UNCOUNTABLE_ru: integer = 1;                 // оепевхякхлнярэ : мер
 const COMPAR_FORM_ru: integer = 26;               { enum яреоемэ }
{ Coordiname яреоемэ states: }
const ATTRIBUTIVE_FORM_ru: integer = 0;            // яреоемэ : юрпха
const COMPARATIVE_FORM_ru: integer = 1;            // яреоемэ : япюбм
const SUPERLATIVE_FORM_ru: integer = 2;            // яреоемэ : опебняу
const LIGHT_COMPAR_FORM_RU: integer = 3;           // яреоемэ : йнлоюпюрхб2
 const CASE_GERUND_ru: integer = 27;               { enum оЮДЕФбЮК }
{ Coordiname оЮДЕФбЮК states: }
 const MODAL_ru: integer = 28;                     { enum лндюкэмши }
 const VERBMODE_TENSE: integer = 29;               { enum яцд_бПЕЛЪ }
{ Coordiname яцд_бПЕЛЪ states: }
 const VERBMODE_DIRECTION: integer = 30;           { enum яцд_мЮОПЮБКЕМХЕ }
{ Coordiname яцд_мЮОПЮБКЕМХЕ states: }
 const NUMERAL_CATEGORY: integer = 31;             { enum йюрецнпхъ_вхяк }
{ Coordiname йюрецнпхъ_вхяк states: }
const CARDINAL: integer = 0;                       // йюрецнпхъ_вхяк : йнкхв
const COLLECTION: integer = 1;                     // йюрецнпхъ_вхяк : янахп
 const ADV_SEMANTICS: integer = 32;                { enum наяр_бюк }
{ Coordiname наяр_бюк states: }
const S_LOCATION: integer = 0;                     // наяр_бюк : леярн
const S_DIRECTION: integer = 1;                    // наяр_бюк : мюопюбкемхе
const S_DIRECTION_FROM: integer = 2;               // наяр_бюк : мюопюбкемхе_нрйсдю
const S_FINAL_POINT: integer = 3;                  // наяр_бюк : йнмевмюъ_рнвйю
const S_DISTANCE: integer = 4;                     // наяр_бюк : пюяярнъмхе
const S_VELOCITY: integer = 5;                     // наяр_бюк : яйнпнярэ
const S_MOMENT: integer = 6;                       // наяр_бюк : лнлемр_бпелемх
const S_DURATION: integer = 7;                     // наяр_бюк : дкхрекэмнярэ
const S_TIME_DIVISIBILITY: integer = 8;            // наяр_бюк : йпюрмнярэ
const S_ANALOG: integer = 9;                       // наяр_бюк : янонярюбкемхе
const S_FACTOR: integer = 10;                      // наяр_бюк : лмнфхрекэ
 const ADJ_TYPE: integer = 33;                     { enum рхо_опхк }
{ Coordiname рхо_опхк states: }
const ADJ_POSSESSIVE: integer = 0;                 // рхо_опхк : опхръф
const ADJ_ORDINAL: integer = 1;                    // рхо_опхк : онпъдй
 const PRONOUN_TYPE_ru: integer = 35;              { enum рхо_леярнхлемхъ }
{ Coordiname рхо_леярнхлемхъ states: }
 const PARTICLE_TYPE: integer = 36;                { enum рхо_вюярхжш }
{ Coordiname рхо_вюярхжш states: }
const PREFIX_PARTICLE: integer = 0;                // рхо_вюярхжш : опетхйя
const POSTFIX_PARTICLE: integer = 1;               // рхо_вюярхжш : оняртхйя
 const ADV_MODIF_TYPE: integer = 37;               { enum рхо_лндхт }
{ Coordiname рхо_лндхт states: }
 const TENSE_en: integer = 44;                     { enum TENSE }
...
const FUTURE_en: integer = 2;                      // TENSE : FUTURE
 const DURATION_en: integer = 45;                  { enum DURATION }
...
const PREPOSITIVE_CASE_en: integer = 1;            // CASE : PREPOSITIVE
 const NOUN_FORM_en: integer = 48;                 { enum NOUN_FORM }
...
const POSSESSIVE_NOUN_FORM_en: integer = 1;        // NOUN_FORM : POSSESSIVE
 const PRONOUN_FORM_en: integer = 49;              { enum PRONOUN_FORM }
{ Coordiname PRONOUN_FORM states: }
 const ADJ_FORM_en: integer = 50;                  { enum ADJ_FORM }
...
const SUPERLATIVE_ADJ_en: integer = 2;             // ADJ_FORM : SUPERLATIVE
 const COMPARABILITY_en: integer = 51;             { enum COMPARABILITY }
...
const NONCOMPARABLE: integer = 3;                  // COMPARABILITY : NONCOMPARABLE
 const VERB_FORM_en: integer = 52;                 { enum VERB_FORM }
...
const TO_MODALITY_en: integer = 2;                 // ENG_MODALITY : To
 const NUMERAL_FORM_en: integer = 55;              { enum NUMERAL_FORM }
...
const ORDINAL_en: integer = 1;                     // NUMERAL_FORM : ORDINAL
 const GENDER_en: integer = 56;                    { enum ENG_GENDER }
...
const FEMININE_en: integer = 1;                    // ENG_GENDER : FEMININE
 const TRANSITIVITY_en: integer = 57;              { enum TRANSITIVITY }
...
const TRANSITIVE_VERB_en: integer = 1;             // TRANSITIVITY : TRANSITIVE
 const OBLIG_TRANSITIVITY_en: integer = 58;        { enum OBLIG_TRANSITIVITY }
 const PROPER_NOUN_en: integer = 59;               { enum ENG_PROPER_NOUN }
 const MASS_NOUN_en: integer = 60;                 { enum ENG_MASS_NOUN }
 const PERSON_fr: integer = 61;                    { enum FR_PERSON }
...
const ORDINAL_fr: integer = 1;                     // FR_NUMERAL_FORM : ORDINAL
 const FR_PRONOUN_FORM: integer = 65;              { enum FR_PRONOUN_FORM }
...
const FR_PRONOUN_STRONG: integer = 1;              // FR_PRONOUN_FORM : STRONG
 const TRANSITIVITY_fr: integer = 66;              { enum FR_TRANSITIVITY }
...
const TRANSITIVE_VERB_fr: integer = 1;             // FR_TRANSITIVITY : TRANSITIVE
 const VERB_FORM_fr: integer = 67;                 { enum FR_VERB_FORM }
...
const IMPERATIVE_fr: integer = 10;                 // FR_VERB_FORM : IMPERATIVE
 const JAP_FORM: integer = 68;                     { enum JAP_FORM }
...
const ROMAJI_FORM: integer = 2;                    // JAP_FORM : ROMAJI
 const JAP_VERB_BASE: integer = 69;                { enum JAP_VERB_BASE }
...
const JAP_VB_CAUSATIVE: integer = 9;               // JAP_VERB_BASE : CAUSATIVE
 const JAP_VERB_KIND: integer = 70;                { enum JAP_VERB_KIND }
...
const JAP_NEGATIVE_IMPERATIVE: integer = 6;        // JAP_VERB_KIND : NEGATIVE_IMPERATIVE
 const JAP_ADJ_BASE: integer = 71;                 { enum JAP_ADJ_BASE }
...
const JAP_AB_PAST: integer = 6;                    // JAP_ADJ_BASE : PAST
 const JAP_ADJ_FORM2: integer = 72;                { enum JAP_ADJ_FORM2 }
...
const JAP_NEGATIVE_PAST_ADJ: integer = 1;          // JAP_ADJ_FORM2 : NEGATIVE_PAST
 const JAP_TRANSITIVE: integer = 73;               { enum JAP_TRANSITIVE }
 const CASE_jap: integer = 74;                     { enum JAP_CASE }
...
const SOCIATIVE_jap: integer = 12;                 // JAP_CASE : SOCIATIVE
 const GENDER_jap: integer = 75;                   { enum JAP_GENDER }
...
const FEMININE_jap: integer = 1;                   // JAP_GENDER : FEMININE
 const PERSON_jap: integer = 76;                   { enum JAP_PERSON }
